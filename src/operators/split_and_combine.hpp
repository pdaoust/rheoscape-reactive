...
namespace rheo::operators {
  ...
  source_fn<TOut> split_and_combine(
  ...
  source_fn<T> split_and_combine(
    ...
    return split_and_combine<T, T1, T1, T2, T2, T>(source, mapper1, pipe1, mapper2, pipe2, combiner);
  }

  template <typename T, typename T1, typename T2>
  pipe_fn<T, T> split_and_combine(
    map_fn<T1, T> mapper1,
    pipe_fn<T1, T1> pipe1,
    map_fn<T2, T> mapper2,
    pipe_fn<T2, T2> pipe2,
