...
namespace rheo::sinks::arduino {
  ...
  struct analog_pin_sink_binder {
    ...
    RHEO_NOINLINE pull_fn operator()(source_fn<int> source) const {
      return source(analog_pin_sink_push_handler{pin, resolution_bits});
    }
  };

  pullable_sink_fn<int> analog_pin_sink(int pin, uint8_t resolution_bits = 10) {
    return analog_pin_sink_binder{pin, resolution_bits};
  }
