...
namespace rheo::sinks {
  ...
  template <typename TKey, typename TValue>
  cap_fn<TableSinkInput<TKey, TValue>> table_sink(
    store_fn<TKey, TValue> store
  ) {
    return [store](source_fn<TableSinkInput<TKey, TValue>> source) {
      table_sink_push_handler<TKey, TValue> handler{store};
      pull_fn pull = source(handler);
  ...
  template <typename TKey, typename TValue>
  pullable_sink_fn<TableSinkInput<TKey, TValue>> table_sink_pullable(
    store_fn<TKey, TValue> store
  ) {
    return [store](source_fn<TableSinkInput<TKey, TValue>> source) -> pull_fn {
      table_sink_push_handler<TKey, TValue> handler{store};
