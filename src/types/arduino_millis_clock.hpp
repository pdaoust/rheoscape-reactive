...
template <>
struct fmt::formatter<rheoscape::arduino_millis_clock_duration> : fmt::formatter<unsigned long> {
  auto format(const rheoscape::arduino_millis_clock_duration& ts, format_context& ctx) const {
    return fmt::formatter<unsigned long>::format(ts.count(), ctx);
  }
};

template <>
struct fmt::formatter<rheoscape::arduino_millis_clock_time_point> : fmt::formatter<unsigned long> {
  auto format(const rheoscape::arduino_millis_clock_time_point& ts, format_context& ctx) const {
