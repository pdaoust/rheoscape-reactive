...
void test_stopwatch_is_contiguous_before_first_lap() {
  ...
  //auto clockSourceAsDuration = map<mock_clock_ulong_millis::duration, mock_clock_ulong_millis::time_point>(clockSource, [](mock_clock_ulong_millis::time_point v) { return v.time_since_epoch(); });
  ...
  auto sw = stopwatch<mock_clock_ulong_millis::duration>(numbersSource, clockSource, [](auto v) { return v > 0; });

  mock_clock_ulong_millis::duration timestamp;
  int pushedValue;
  auto pull = sw(
    [&timestamp, &pushedValue](auto v) {
      timestamp = v.tag;
      pushedValue = v.value;
    }
  );
